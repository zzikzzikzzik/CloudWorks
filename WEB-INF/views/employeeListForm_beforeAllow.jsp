...
<script>
	...
	$(document).ready(function(){
		...
		inputData(".selectJikupCode", "${requestScope.employeeSearchDTO.selectJikupCode}");
		...
		//------------------------------------------------------------------------
 		...
 		$(".pagingNumber").html(
				getPagingNumber(
					"${requestScope.beforeAllowCnt}"		// 검색 결과 총 갯수
	...
	function goEmployeeUpDelForm(emp_no){
	 	$("[name=employeeUpDelForm] [name=selectPageNo]").val($("[name=employeeListForm_beforeAllow] [name=selectPageNo]").val());
		$("[name=employeeUpDelForm] [name=rowCntPerPage]").val($("[name=employeeListForm_beforeAllow] [name=rowCntPerPage]").val());
		$("[name=employeeUpDelForm] [name=keyword1]").val($("[name=employeeListForm_beforeAllow] [name=keyword1]").val());
...
<div class="container">
	...
	<h3 style="text-align: center;"> 가입관리 </h3>
	<br>
	<form name="employeeListForm_beforeAllow" method="post" action="${requestScope.croot}/employeeListForm_beforeAllow.do">
		<table border=0 width=100%>
			...
			<select name="selectDepNo" class="selectDep_no form-control" style="display:inline-block; width:auto; height:auto; padding: 2px 10px;">
					<option value="">소속부서
					<option value="1">총무부
					<option value="2">영업부
					<option value="3">자재부
		...
		<tr><td><br>
			검색된 총 갯수 : <span><strong>${requestScope.beforeAllowCnt}</strong></span>&nbsp;
		...
		<table class="table table-striped table-hover">
			...
			<c:forEach items="${requestScope.beforeAllow}" var="beforeAllow" varStatus="loopTagStatus">
				<tr style="cursor:pointer" onClick="goEmployeeUpDelForm(${beforeAllow.emp_no});">
					<td align="center">${beforeAllowCnt-(employeeSearchDTO.selectPageNo*employeeSearchDTO.rowCntPerPage-employeeSearchDTO.rowCntPerPage+1+loopTagStatus.index)+1 }
					<td align="center">${beforeAllow.emp_no}
					<td align="center">${beforeAllow.emp_name}
					<td align="center">${beforeAllow.jikup_name}
					<td align="center">${beforeAllow.dep_name}
					<td align="center">${beforeAllow.mgr_emp_no}
					<td align="center">${beforeAllow.mgr_emp_name}
					<td align="center">${beforeAllow.phone_num}
					<td align="center">${beforeAllow.allow_code}		
			</c:forEach>
			<c:if  test="${empty requestScope.beforeAllow}">
		...
		<tr align=center>
			<td>
				<input type="button" onClick="goEmployeeListForm();" value="     직원 관리     " class="btn btn-default" style='cursor:pointer; padding:0px; white-space:pre; height:25px; font-size:12px;'>
	</table>
	</form>
	...
	<form name="employeeUpDelForm" method="post" action="${requestScope.croot}/employeeUpDelForm.do">
		...
		<input type="hidden" name="is_allowed" value="false">
	</form>
